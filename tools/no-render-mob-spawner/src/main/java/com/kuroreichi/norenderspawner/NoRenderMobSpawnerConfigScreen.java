...
public final class NoRenderMobSpawnerConfigScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.centeredText(this.font, Component.literal("Targeting a hidden spawner still shows the selection outline"), this.width / 2, 43, 0xFFAAAAAA);
        graphics.fill(left, 54, left + width, 180, 0x50000000);
        graphics.outline(left, 54, width, 126, 0x80FFFFFF);
        graphics.centeredText(this.font, Component.literal("Skip Render Updates reduces client-side spawner visual work"), this.width / 2, 192, 0xFFAAAAAA);
    }
