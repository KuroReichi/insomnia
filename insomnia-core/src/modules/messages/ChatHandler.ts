...
if (configs["command.prefix"].startsWith("/")) {
	...
	function CommandQueue(player: Player, query: string[]) {
	...
	world.beforeEvents.chatSend.subscribe((e) => {
		e.cancel = true;
		const player: Player = e.sender;
		const PlayerDB: Database = new Database(player.name);
		if (e.message.startsWith(configs["command.prefix"])) {
			let command: RegExpMatchArray | [] =
				e.message
					.slice(configs["command.prefix"].length)
					.trim()
					.match(/"[^"]*"|'[^']*'|`[^`]*`|\S+/g) ?? [];
			let query: string[] = command.map((v) => v.replace(/^["'`]|["'`]$/g, ""));
